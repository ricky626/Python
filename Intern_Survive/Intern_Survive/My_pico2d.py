...
def open_canvas(w=int(1024), h=int(768), sync=True):
    ...
    caption = ('Intern_Survive (' + str(w) + 'x' + str(h) + ')' + ' 1000.0 FPS').encode('UTF-8')
...
def print_fps():
    ...
    cur_time += dt
    dt = max(dt, 0.0001)
    caption = ('Intern_Survive (' + str(canvas_width) + 'x' + str(canvas_height) + ')' + ' %4.2f FPS' % (1.0/dt)).encode('UTF-8')
    #caption = ('Color Trick (' + str(canvas_width) + 'x' + str(canvas_height) + ')').encode('UTF-8')
